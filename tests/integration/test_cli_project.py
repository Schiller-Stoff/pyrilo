...
def test_delete_project_success(mock_pyrilo_ingest_env):
    """
    Verifies 'delete_project' sends the correct DELETE request.
    """
    gams_api_mock, test_pyrilo_project = mock_pyrilo_ingest_env
    ...
    gams_api_mock.delete(f"{test_pyrilo_project.TEST_HOST}/api/v1/projects/{test_pyrilo_project.TEST_PROJECT}", status_code=200)

    result = runner.invoke(cli, [
        "--host", test_pyrilo_project.TEST_HOST,
        "delete_project", test_pyrilo_project.TEST_PROJECT
    ], env={"PYRILO_USER": "u", "PYRILO_PASSWORD": "p"})

    assert result.exit_code == 0
    assert "Successfully deleted project" in result.output

    # Verify the API call details
    history = gams_api_mock.request_history
    delete_requests = [r for r in history if r.method == "DELETE" and r.url.endswith(f"/projects/{test_pyrilo_project.TEST_PROJECT}")]

    assert len(delete_requests) == 1



def test_delete_project_fails_if_exists(mock_pyrilo_ingest_env):
    """
    Verifies correct error handling when GAMS returns 409 Conflict.
    """
    gams_api_mock, test_pyrilo_project = mock_pyrilo_ingest_env
    ...
    # Override the default 201 response with a 409 Conflict
    gams_api_mock.delete(f"{test_pyrilo_project.TEST_HOST}/api/v1/projects/{test_pyrilo_project.TEST_PROJECT}", status_code=409)
    ...
    # Expecting exit code 1 because cli.py catches Exception and calls sys.exit(1)
    assert result.exit_code == 1
    assert "Failed to delete project" in result.output